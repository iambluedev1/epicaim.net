...
class View
{
    ...
    public static function render(array $array)
    {
        ...
        $online = new Online();
        $geo = new Geolocation();
        $state = $online->isConnected($geo->getIp(), "onlineIp");
        $datas = array(
			"userId" => (Session::readUser("token") == null) ? -1 : Session::readUser("id"),
            "onlineIp" => $geo->getIp(),
            "pageName" => (self::$data != null) ? (@(self::$data["title"] != null) ? self::$data["title"] : "Page Inconnue") : "Page Inconnue",
			"onlineState" => (Session::readUser("token") == null) ? 0 : 1,
			"pageUrl" => Url::detectUri(),
			"onlineTime" => time(),
        );
        if(!$state){
			$online->insert($datas);
		}else{
			$online->update($datas, $state);
		}
